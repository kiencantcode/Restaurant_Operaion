...
public:
	...
	void heapify_down(vector<Slot> &heap, int i)
	{
		...
		int smallest = i;
		if (left_child_index < n && ((heap[left_child_index].num < heap[smallest].num) || ((heap[left_child_index].num == heap[smallest].num) && (heap[left_child_index].time < heap[smallest].time))))
		{
			smallest = left_child_index;
		}

		if (right_child_index < n && ((heap[right_child_index].num < heap[smallest].num) || ((heap[right_child_index].num == heap[smallest].num) && (heap[right_child_index].time < heap[smallest].time))))
		{
	...
	void insert(vector<Slot> &heap, string name, int result, int ID, int num, int time)
	{
		...
		slot.num = num;
		slot.time = time;
		// cout << "Inserting: " << ID << " with num " << num << endl;
		heap.push_back(slot);
	...
	void delete_value_heap_ID(vector<Slot> &heap, int ID)
	{
		...
		// cout << "Deleting: " << ID << " with num " << heap[index].num << endl;
	...
	void display_test(vector<Slot> &heap, int i)
	{
		signed int n = heap.size();
		if (i < n)
		{
			cout << heap[i].ID << "-" << heap[i].num << endl;
			display_test(heap, 2 * i + 1);
			display_test(heap, 2 * i + 2);
		}
	}

	void printHeap2D(vector<Slot> &heap, int i, int space)
	{
		signed int n = heap.size();
		if (i < n)
		{
			space += 10;
			printHeap2D(heap, 2 * i + 2, space);
			cout << endl;
			for (int j = 10; j < space; j++)
				cout << " ";
			cout << heap[i].ID << "-" << heap[i].num << endl;
			printHeap2D(heap, 2 * i + 1, space);
		}
	}
};
...
public:
	...
	void clear(HashTable &zone1)
	{
		...
		for (unsigned int i = 0; i < slot_1.size(); i++)
		{
			...
			if(checkZone1 == true){
				...
				i--;
				for (unsigned int k = 0; k < slot_2.size(); k++)
				{
...
public:
	...
	Node *deleteNode(Node *node, int result, int ID)
	{
		...
		else if (result == node->result && ID != node->ID){
			node->left = deleteNode(node->left, result, ID);
			node->right = deleteNode(node->right, result, ID);
		}
		else if (result == node->result && ID == node->ID){
	...
	void deleteOrderDish(int ID)
	{
		for (unsigned int i = 0; i < order_dish.heap.size(); i++)
		{
			if (order_dish.heap[i].ID == ID){
				order_dish.delete_value_heap(order_dish.heap, order_dish.heap[i].name);
				return;
			}
		}
	}
	...
	void helperClearSlot(Node *root)
	{
		for (unsigned int i = 0; i < slot_1.size(); i++)
		{
			...
			{
				deleteOrderDish(slot_1[i].ID);
				slot_1.erase(slot_1.begin() + i);
				i--;
			}
		}
		for (unsigned int j = 0; j < slot_2.size(); j++)
		{
...
void HuffmanCodes(char data[], int freq[], int size)
{
	...
	if(buildMinHeap.size() == 1 && buildMinHeap[0]->data != '$'){
        right = buildMinHeap[0];
        buildMinHeap.erase(buildMinHeap.begin());
...
void FIFO(vector<Slot> &slot_1, vector<Slot> &slot_2, string name, int res, HashTable &zone1, AVLTree &zone2, Node *&node)
{
	...
	// xoa ben cua slot 2
	for (unsigned int i = 0; i < slot_2.size(); i++)
	{
...
void LRCO(vector<Slot> &slot_2, vector<Slot> &slot_1, string name, int res, int ID, HashTable &zone1, AVLTree &zone2, Node *&node)
{
...
void LFCO(Heap &order_dish, vector<Slot> &slot_1, string name, int res, int ID, HashTable &zone1, AVLTree &zone2, Node *&node)
{
	// int min_order_dish = order_dish.heap[0].num;
	// int min_order_time = order_dish.heap[0].time;
	int delete_ID = order_dish.heap[0].ID;
	string name_delete;
	int delete_res;
	int save_i;
	int save_j;
...
bool isNum(string num){
	int n = num.length();
	for (int i = 0; i < n; i++){
		if (isdigit(num[i]) == false) return false;
	}
	return true;
}
...
void order(string name, Node *&root, HashTable &zone1, AVLTree &zone2)
{
	...
	else if (count == MAXSIZE && name_insert_1 == name && name_insert_2 == name)
	{
		...
		for (unsigned int i = 0; i < slot_1.size(); i++)
		{
			if (slot_1[i].name == name)
			{
				...
				int ID_insert = slot_1[i].ID;
				zone1.updateNum(ID_insert, update_num);
				zone2.updateNum(root, ID_insert, update_num);
				order_dish.delete_value_heap(order_dish.heap, name);
				order_dish.insert(order_dish.heap, name, res, ID_insert, update_num, time_insert);
			}
		}
		for (unsigned int i = 0; i < slot_2.size(); i++)
		{
			...
			if (checkZone1 == false && checkZone2 == false)
			{
				...
				if (count == MAXSIZE / 2)
				{
					...
					if (count == 0)
					{
						...
						order_dish.insert(order_dish.heap, name, res, ID, 1, time_insert);
					...
					// 	cout << slot_2[i].name << " " << slot_2[i].result << " " << slot_2[i].ID << " " << slot_2[i].num << endl;
					// }
					break;
				}
...
void clear(Node *&root, HashTable &zone1, AVLTree &zone2, int num, Heap &order_dish)
{
...
void printHT(HashTable &zone1)
{
	zone1.display();
}

void printAVL(Node *&root, AVLTree &zone2)
{
	zone2.display(root);
	// cout << "--------------------------" << endl;
}
